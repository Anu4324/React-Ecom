...
function getAPIData() {
dispatch(getBrand()); // Corrected to invoke getBrand as a function
  ...
  <div className="container-fluid page-header py-5">
    ...
    <ol className="breadcrumb justify-content-center mb-0">
      ...
      <li className="breadcrumb-item active text-white">
        <Link to="/AdminBrand" className="text-white">AdminBrand</Link>
      </li>
    </ol>
  </div>
  {/*
          ...
          <table className="table table-bordered table-hover">
            <tbody>
              <tr className="text-center">
                ...
                <th className="text-blacks">Logo</th>
              ...
              <tr key={index} className="text-center">
               <td>{item.id}</td>
                <td>{item.name}</td>
                <td>
                  <img src={`/img/logos/${item.pic}`} style={{ height: "50px", width: "60px" }} alt={item.pic
                    || "Logo Image" }/>
                </td>
                <td>
                  <Link to={"/AdminUpdateBrand/" + item.id} className="text-primary">
